...
class CircuitBreaker:
    def __init__(self, failure_threshold: int = 3, recovery_timeout: int = 5):
        ...
        Args:
            ...
            recovery_timeout: Segundos para esperar antes de tentar de novo
            (Half-Open).
        """
    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if self.state == State.OPEN:
            ...
            if elapsed > self.recovery_timeout:
                logger.warning(
                    "⚠️ Tempo de recuperação passou. Mudando para HALF-OPEN.")
                self.state = State.HALF_OPEN
            else:
                # Falha rápida (Fail Fast)
                raise CircuitBreakerOpenException(
                    f"Circuito ABERTO. Tente novamente em"
        ...
        try:
            ...
            if self.state == State.HALF_OPEN:
                logger.success(
                    "✅ Recuperação confirmada! Fechando o circuito.")
                self.reset()
    ...
    def _handle_failure(self):
        ...
        logger.error(f"❌ Falha detectada ({self.failure_count}"
                     f"/{self.failure_threshold})")

        if self.state == State.HALF_OPEN:
            logger.error(
                "⛔ Falha na recuperação (Half-Open). Voltando para OPEN.")
            self.state = State.OPEN

        elif self.failure_count >= self.failure_threshold:
            logger.error(
                "💥 Limite de falhas atingido! Abrindo o circuito (OPEN).")
            self.state = State.OPEN
