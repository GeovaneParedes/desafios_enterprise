...
app = FastAPI(title="Enterprise Challenge #03 - Distributed Lock", lifespan=lifespan)
    ...
    if lock.acquire(blocking=True, timeout=5):
        try:
            ...
            if estoque > 0:
                ...
                print(f"✅ Venda realizada para {request.user_id}! Estoque restante: {novo_estoque}")
                return {
                    "status": "sucesso", 
                ...
                print(f"❌ Usuário {request.user_id} tentou comprar, mas acabou.")
                raise HTTPException(status_code=409, detail="Esgotado! Você chegou tarde.")
        ...
        raise HTTPException(status_code=429, detail="O sistema está congestionado. Tente novamente.")
